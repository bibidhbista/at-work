...
SELECT [Session ID] = s.session_id, 
       ...
       [Head Blocker] = CASE -- session has an active request, is blocked, but is blocking others

                            WHEN r2.session_id IS NOT NULL
                                 AND r.blocking_session_id = 0 THEN '1' -- session is idle but has an open tran and is blocking others
 ...
 r.total_elapsed_time / (60*1000.0) 'Elapsed Time (in Minutes)',
...
WHERE s.session_Id > 50 AND t.[task_state] !='' -- Ignore system spids.
