---------------------------------------------------------------------- Creates all required tables/sps/jobs for IndexOptimize Solution by Ola Hallengren --------------------------------------------------------------------------------------
...
/****** Object:  Table [dbo].[CommandLog]    Script Date: 10/23/2017 9:36:57 AM ******/
SET ANSI_NULLS ON
GO
...
PRINT 'CommandLog Table Created'
...
/****** Object:  StoredProcedure [dbo].[CommandExecute]    Script Date: 10/23/2017 9:39:20 AM ******/
SET ANSI_NULLS ON
GO
...
PRINT 'Command execute SP Created'




---------------------------------------------------------------------------------------------------------------------
--------------------------------------------------- DBIntegrity Check Creation --------------------------------------
---------------------------------------------------------------------------------------------------------------------
...
/****** Object:  StoredProcedure [dbo].[DatabaseIntegrityCheck]    Script Date: 4/16/2018 9:52:58 AM ******/
SET ANSI_NULLS ON
GO
...
PRINT 'DBIntegrityCheck SP Created'
...
/****** Object:  StoredProcedure [dbo].[IndexOptimize]    Script Date: 10/23/2017 9:39:23 AM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  Job [IndexOptimize - USER_DATABASES]    Script Date: 4/16/2018 10:09:19 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 4/16/2018 10:09:19 AM ******/
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'IndexOptimize - USER_DATABASES', 
		...
		@command=N'sqlcmd -E -S $(ESCAPE_SQUOTE(SRVR)) -d master -Q "EXECUTE dbo.IndexOptimize @Databases = ''USER_DATABASES'', @PageCountLevel = 1000, @FragmentationLow = NULL, @FragmentationMedium = ''INDEX_REORGANIZE,INDEX_REBUILD_ONLINE,INDEX_REBUILD_OFFLINE'', @FragmentationHigh = ''INDEX_REBUILD_ONLINE,INDEX_REBUILD_OFFLINE'', @FragmentationLevel1 = 5, @FragmentationLevel2 = 30, @TimeLimit = 3600, @LogToTable = ''Y''" -b', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Schedule', 
		...
		@schedule_uid=N'7f8aa85e-3049-4550-ad0f-d724f4710bd4'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
PRINT 'IndexOptimize Job Created'
...
------------------------------------------------------------------------------------------------------------------
------------------------------------- Weekly_UserDbs_Maintenance Job Creation ------------------------------------
------------------------------------------------------------------------------------------------------------------
...
/****** Object:  Job [Weekly_UserDBs_Maintenance.Subplan_1]    Script Date: 4/16/2018 9:58:01 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 4/16/2018 9:58:01 AM ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'Database Maintenance' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'Database Maintenance'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Weekly_UserDBs_Maintenance.Subplan_1', 
		@enabled=1, 
		@notify_level_eventlog=2, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Subplan_1', 
		...
		@command=N'/Server "$(ESCAPE_NONE(SRVR))" /SQL "Maintenance Plans\Weekly_UserDBs_Maintenance" /set "\Package\Subplan_1.Disable;false"', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Weekly_UserDBs_Maintenance.Subplan_1', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@active_end_time=235959, 
		@schedule_uid=N'8c4ca02e-6e4b-4427-b57b-24070e33543e'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
PRINT 'Weekly_UserDBs_Maint Job Created'
...
/****** Object:  Job [CommandLog Cleanup]    Script Date: 10/23/2017 10:20:36 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 10/23/2017 10:20:36 AM ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'Database Maintenance' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'Database Maintenance'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [CommandLog Cleanup]    Script Date: 10/23/2017 10:20:36 AM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'CommandLog Cleanup', 
		...
		@output_file_name=N'F:\MSSQL13.MSSQLSERVER\MSSQL\Log\CommandLogCleanup_$(ESCAPE_SQUOTE(JOBID))_$(ESCAPE_SQUOTE(STEPID))_$(ESCAPE_SQUOTE(STRTDT))_$(ESCAPE_SQUOTE(STRTTM)).txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
PRINT 'CommandCleanup Job Created'
