...
CREATE TABLE db_principals_statements (stmt varchar(max), result_order decimal(4,1))
...
EXEC ('
INSERT INTO db_principals_statements (stmt, result_order)
      SELECT      
      ...
      WHERE [type] IN (''U'', ''S'', ''G'') AND [name] not in (''guest'',''sa'')/* windows users, sql users, windows groups */')
...
CREATE TABLE db_permissions ([--stmt] varchar(max), sort int)
...
BEGIN
        INSERT INTO db_permissions ([--stmt],sort)
		VALUES (@sql, @sort)
	--	PRINT @sql
        FETCH NEXT FROM tmp INTO @sql, @sort    
END
...
SELECT [--stmt] FROM dbo.db_permissions

DROP TABLE db_principals_statements
