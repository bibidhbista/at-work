...
try{
    ...
    #$backupDirectory='\\pfs02\sqlbackup\dbaTools_Staging\For_Refresh\pdmsql12_to_ddmsql12'
...
try{
    ...
    if($count -gt 0){
       ...
       #foreach($user in $users){
              # Copy-DbaLogin -Source $SourceServer -Destination tdmsql12  -Login $user # Migrates Logins with password so they don't have to be reentered for SQL Logins
       #}
